-- ==============================================================================
-- 12_SALES_SUMMARY.SQL
-- DASH-03 · Bloco 2 · Resumo de vendas (RF037)
-- Dependência: 05_sales_schema.sql, 03_inventory_schema.sql
-- ==============================================================================

-- Suporte às agregações por período (organização + status + data de venda,
-- que é finalized_at quando existe — pedidos online — ou created_at — balcão,
-- que já nasce 'confirmed' e nunca recebe finalized_at).
...
-- ==============================================================================
-- GET_SALES_SUMMARY (RF037 · RN090/RN091)
-- Soma vendas de balcão (channel='pos', já nasce 'confirmed') + pedidos
-- online confirmados (channel='catalog_store', status='confirmed') no
-- período informado. Recorte por papel aplicado no servidor: Sales só
-- recebe own_sales_today; margem/inventário só para Owner.
-- ==============================================================================
...
BEGIN
  ...
  -- RN091: Sales só recebe o contador das próprias vendas do dia — nada de
  -- série/consolidado da organização.
  ...
  -- Série do gráfico de área empilhado: um ponto por dia (ou por hora em
  -- "Hoje"), com as duas séries — balcão × pedidos — já separadas.
  ...
  -- Owner exclusivo (RN091): inventário a custo (Σ stock × cost_price da
  -- org, produtos simples + variantes) e margem média do período
  -- (faturamento − CMV, pelo cost_price atual dos itens vendidos).
