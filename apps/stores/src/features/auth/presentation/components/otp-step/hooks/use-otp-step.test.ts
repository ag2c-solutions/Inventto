...
describe('useOtpStep', () => {
  ...
  it('deve chamar onSubmit com o code quando ele tem 8 dígitos', () => {
    const { result } = renderUseOtpStep();

    act(() => {
      result.current.handleCodeChange('87654321');
    });
    ...
    expect(onSubmit).toHaveBeenCalledWith('87654321');
    expect(onSubmit).toHaveBeenCalledTimes(1);
  });

  it('não deve chamar onSubmit quando o code tem menos de 8 dígitos', () => {
    const { result } = renderUseOtpStep();
