...
describe('ProductFormFieldCategory (Integration)', () => {
  ...
  it('should keep the popover open and not crash when the create mutation fails', async () => {
    mocks.createCategory.mockRejectedValue(new Error('Falha ao criar'));
    const user = userEvent.setup();
    ...
    expect(combobox).not.toHaveTextContent('Sapatos');
  });
});
