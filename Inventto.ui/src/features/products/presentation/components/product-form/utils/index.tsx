...
export const getCombinationKey = (options: VariantOption[]): string => {
  return [...options]
...
export const generateVariants = ({
  ...
  for (const attribute of attributes) {
    const values = attribute.values.filter((v) => v.trim().length > 0);
...
export const parseValues = (
  valueString: string | string[] | undefined
): string[] => {
  if (!valueString) return [];

  if (Array.isArray(valueString)) return valueString;
...
export const generateAttributeSlug = (text: string) => {
  return text
    .toString()
    .toLowerCase()
    .normalize('NFD')
