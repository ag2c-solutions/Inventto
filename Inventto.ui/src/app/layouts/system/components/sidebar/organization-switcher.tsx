...
export function OrganizationSwitcher({ className }: { className?: string }) {
  const { organization, availableOrganizations, setCurrentOrganization } = useUser();
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          className={cn("w-full justify-between p-2 hover:bg-accent/50 h-auto", className)}
        >
          <div className="flex items-center gap-3 truncate">
            ...
            <div className="flex flex-col items-start truncate text-sm">
                ...
                <span className="text-xs text-muted-foreground capitalize">
                    {organization.role === 'owner' ? 'Proprietário' : organization.role}
                </span>
            </div>
          </div>
