...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const currentOrganization = useMemo(() => {
    ...
    const found = user.availableOrganizations.find(org => org.id === selectedOrgId);
  ...
  const handleSetOrganization = useCallback((orgId: string) => {
    ...
    const isValid = user.availableOrganizations.some(org => org.id === orgId);
    
    if (!isValid) {
      console.warn(`Tentativa de acesso não autorizado à organização: ${orgId}`);
      return;
    }
  ...
  }), [user, isLoading, isError, error, refetch, currentOrganization, handleSetOrganization]);
