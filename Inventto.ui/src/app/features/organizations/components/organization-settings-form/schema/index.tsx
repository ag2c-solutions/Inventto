...
const dayScheduleSchema = z.object({
  ...
  close: z.string().optional(),
    ...
    if (!data.open || !timeRegex.test(data.open)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Horário de abertura obrigatório", path: ["open"] });
    }
    if (!data.close || !timeRegex.test(data.close)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Horário de fechamento obrigatório", path: ["close"] });
    }
  }
});
...
export const organizationSettingsSchema = z.object({
  ...
  operational: z.object({
    timezone: z.string({ error: "Selecione o fuso horário da loja" })
      .min(1, "Obrigatório"),
    
    // REMOVIDO: .transform()
    whatsappMain: z.string()
      .min(10, "Número inválido (mínimo DDD + 8 dígitos)"), 
      
    // REMOVIDO: .transform()
    whatsappSupport: z.string()
      .optional(),
  }),
