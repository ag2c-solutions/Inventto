...
export const getMovementItemImage = (
  ...
  const variantImgRef = variant?.images?.find((img) => img.isPrimary) || variant?.images?.[0];
  const variantImage = variantImgRef ? allImages.find((img) => img.id === variantImgRef.id) : null;
